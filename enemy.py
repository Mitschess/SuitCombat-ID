...
from settings import (
    ...
    MELEE_RANGE, MELEE_COOLDOWN, MELEE_DURATION,
    RANGED_DAMAGE, RANGED_SPEED, RANGED_COOLDOWN,
    COLOR_ENEMY, COLOR_ENEMY_ACCENT, COLOR_WHITE
)

class Enemy:
    def __init__(self, x=SCREEN_WIDTH - 200 - 60, y=GROUND_Y - 90, character=None):
        self.x = float(x)
        self.y = float(y)
        self.vx = 0.0
        self.vy = 0.0
        ...
        self.facing_direction = -1  # Default faces left towards Player
        ...
        self.accent_color = COLOR_ENEMY_ACCENT
        self.name = "COMPUTER"
    ...
    def load_sprites(self):
        """Attempts to load enemy character sprite image if provided in assets (FR-14 compliance)."""
        sprite_path = os.path.join(os.path.dirname(__file__), "assets", "characters", "enemy", "enemy.png")
    ...
    def move_left(self):
        if not self.is_blocking and not self.is_crouching and not self.is_dead:
            self.vx = -FIGHTER_SPEED * self.speed_factor  # difficulty-adjusted AI speed

    def move_right(self):
        if not self.is_blocking and not self.is_crouching and not self.is_dead:
            self.vx = FIGHTER_SPEED * self.speed_factor
    ...
    def crouch(self, enable=True):
        if self.is_grounded:
            self.is_crouching = enable
            if enable:
                self.is_blocking = False

    def block(self, enable=True):
        if self.is_grounded and not self.is_crouching:
            self.is_blocking = enable
            if enable:
                self.vx = 0.0
    ...
    def attack_ranged(self):
        if self.ranged_cooldown == 0 and not self.is_blocking and not self.is_dead and self.ulti_cast_timer == 0:
            self.cast_timer = 16
            self.ranged_cooldown = RANGED_COOLDOWN + 10  # slight AI delay
            ...
            return {
                ...
                'damage': RANGED_DAMAGE * self.damage_mult,
                'speed': RANGED_SPEED
            }
        return None
    ...
    def draw(self, surface):
        ...
        # 3. Draw styled mockup character (Crimson Cyber Fighter)
        draw_color = (255, 255, 255) if self.hit_flash_timer > 0 else self.color
        ...
        if self.is_blocking:
            ...
            pygame.draw.rect(surface, (255, 100, 100), shield_rect, border_radius=4)
